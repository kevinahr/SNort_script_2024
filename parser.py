...
pcap_file = 'your_pcap_file.pcap'  # Replace with your actual pcap file
...
def parse_pcap(pcap_file):
    ...
    for packet in packets:
        ...
        if DNS in packet:
            parse_data['dns_query'] = packet[DNSQR].qname.decode('utf-8')
            parse_data['dns_response'] = packet[DNSQR].rdata
            parse_data['dns_query_type'] = packet[DNSQR].qtype
            if len(packet[DNS].payload) > 100:
                parse_data['Suspicious_dns_tunneling'] = packet.summary()
        if Raw in packet:
